    ...
    <form action="/admin/penjual/{{ $user->id }}" method="POST" autocomplete="off">
        ...
        <div class="space-y-4">
            ...
            <div>
                ...
                <input type="email" name="email" value="{{ old('email', $user->email) }}" placeholder="email@stand.com"
                       class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:border-green-500 focus:outline-none"/>
            </div>
            <div>
                ...
                <select name="stand_id" class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:border-green-500 focus:outline-none">
                    ...
                    @foreach($stands as $s)
                    <option value="{{ $s->id }}" {{ old('stand_id', $user->stand_id) == $s->id ? 'selected' : '' }}>
                        {{ $s->emoji }} {{ $s->nama }}
                    </option>
                    @endforeach
                </select>
            ...
            <div>
                <label class="block text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1.5">
                    Password <span class="text-gray-300 font-normal">(kosongkan jika tidak ingin diubah)</span>
                </label>
