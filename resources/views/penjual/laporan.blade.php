...
<div class="bg-white rounded-2xl border border-gray-100 p-4 mb-6">
    <form method="GET" action="/penjual/laporan" class="flex gap-3">
...
<div class="bg-white rounded-2xl border border-gray-100 p-6">
    ...
    <p class="text-gray-400 text-sm text-center py-8">Belum ada data untuk periode ini</p>
    @else
    <div class="space-y-3">
        ...
        <div class="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
            ...
            <div class="text-right">
                <p class="font-bold text-sm text-gray-800">{{ $m['qty'] }}x terjual</p>
...
<div class="bg-white rounded-2xl border border-gray-100 p-6 mt-6">
    <h2 class="font-bold text-gray-800 mb-4">⭐ Ulasan Terbaru</h2>
    @if($ulasan->isEmpty())
    <p class="text-gray-400 text-sm text-center py-8">Belum ada ulasan</p>
    @else
    <div class="space-y-3">
        @foreach($ulasan as $u)
        <div class="bg-gray-50 rounded-xl p-4">
            <div class="flex items-center justify-between mb-2">
                <div class="flex items-center gap-2">
                    <span class="text-lg">{{ $u->menu->emoji ?? '🍽️' }}</span>
                    <div>
                        <p class="font-medium text-sm text-gray-800">{{ $u->menu->nama ?? '-' }}</p>
                        <p class="text-xs text-gray-400">{{ $u->mahasiswa->nama ?? '-' }} · {{ $u->created_at->format('d M Y') }}</p>
                    </div>
                </div>
                <div class="flex">
                    @for($i = 1; $i <= 5; $i++)
                    <span class="{{ $i <= $u->rating ? 'text-yellow-400' : 'text-gray-200' }} text-sm">★</span>
                    @endfor
                </div>
            </div>
            @if($u->komentar)
            <p class="text-sm text-gray-600 mt-1">{{ $u->komentar }}</p>
            @endif
        </div>
        @endforeach
    </div>
