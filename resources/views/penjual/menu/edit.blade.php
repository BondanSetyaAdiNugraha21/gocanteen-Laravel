        ...
        <div class="space-y-4">
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="2" placeholder="Deskripsi menu..."
                          class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:border-green-500 focus:outline-none resize-none">{{ old('deskripsi', $menu->deskripsi) }}</textarea>
            </div>
            <div class="flex gap-4">
                <div class="flex-1">
                    <label class="block text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1.5">Harga *</label>
                    <input type="number" name="harga" value="{{ old('harga', $menu->harga) }}" placeholder="15000" min="0"
                           class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:border-green-500 focus:outline-none"/>
                </div>
                <div class="flex-1">
                    <label class="block text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1.5">Stok *</label>
                    <input type="number" name="stok" value="{{ old('stok', $menu->stok) }}" placeholder="50" min="0"
                           class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:border-green-500 focus:outline-none"/>
                </div>
            </div>
            <div>
                <label class="block text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1.5">Emoji Menu</label>
                <input type="hidden" name="emoji" id="emojiInput" value="{{ old('emoji', $menu->emoji) }}"/>
                <div class="relative">
                    <button type="button" onclick="toggleEmojiPicker()"
                        ...
                        <span id="emojiPreview" class="text-2xl">{{ old('emoji', $menu->emoji) }}</span>
            ...
            <div class="flex items-center gap-3">
                <input type="checkbox" name="tersedia" id="tersedia" {{ $menu->tersedia ? 'checked' : '' }} class="w-4 h-4 accent-green-700"/>
